...
# [삭제됨] aws_ec2_transit_gateway_route (BGP가 자동으로 전파하므로 삭제)
...
resource "null_resource" "run_ansible" {
  triggers = {
    playbook_hash = filesha256("setup_windows_vpn.yml")
  ...
  depends_on = [
    ...
    # [수정됨] 삭제된 onprem_static_route 의존성 제거
  ]

  provisioner "local-exec" {
    command = <<EOT
      ansible-playbook -i inventory.ini setup_windows_vpn.yml \
        --extra-vars 'ssm_code="${aws_ssm_activation.windows_onprem.activation_code}" ssm_id="${aws_ssm_activation.windows_onprem.id}" tunnel1_ip="${aws_vpn_connection.vpn.tunnel1_address}" tunnel1_psk="${data.aws_ssm_parameter.tunnel1_psk.value}"'
    EOT
  }
}
